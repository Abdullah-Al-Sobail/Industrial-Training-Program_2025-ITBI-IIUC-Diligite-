<?php
  include './env.php';
  $stmt=$con->prepare("SELECT title, description, btn_text, heroImg FROM heroposts WHERE status=?");
  $status = 1;
...
$mainCategory = 'main catego';
$stmt2=$con->prepare("SELECT * FROM categories WHERE name=?");
$stmt2->bind_param('s',$mainCategory);
  $stmt2->execute();

$fetch2 = $stmt2->get_result()->fetch_all(1);
  // var_dump($fetch2);
...
$grouped =[
  // NULL=>[
  //   [0]=>['id'=>1,'name'=>'category1'],
  //   [1]=>['id'=>2,'name'=>'category2'],
  //   [1]=>['id'=>3,'name'=>'category2'],
  
  // ],
  // 2=>[
  //   'id'=>2,'name'=>'sUBcategory3'
  // ]
];
// $test = [];
// $grouped[NULL][] = $test;
...
// $categories_sample = [
//  ['id'=>1,'name'=>'category1','parent_id'=>NULL],
//  ['id'=>2,'name'=>'category2','parent_id'=>NULL],
//  ['id'=>3,'name'=>'category3','parent_id'=>NULL],
//  ['id'=>3,'name'=>'sUBcategory3','parent_id'=>2],
//  ['id'=>3,'name'=>'subcategory3','parent_id'=>3],
...
<head>
	...
	<link rel="shortcut icon" href="assetsassets/images/favicon.png" type="image/x-icon">
...
<header class="navigation bg-tertiary">
	<nav class="navbar navbar-expand-xl navbar-light text-center py-3">
		<div class="container">
          ...
          <?php
            foreach($grouped[NULL] as $main){

              ?>
          	<li class="nav-item <?= isset($grouped[$main['id']]) ? 'dropdown':''?>"> <a class="nav-link " href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false"><?=$main['name']?></a>
           <?php if(isset($grouped[$main['id']])){?>
                	<ul class="dropdown-menu" aria-labelledby="navbarDropdown">
                    <?php
                    foreach($grouped[$main['id']] as $sub){?>
                    <li><a class="dropdown-item " href="blog-details.html"><?=$sub['name']?></a>
							</li>
    ...
    <div class="row align-items-center justify-content-center">
      <div class="col-lg-6 mb-5 mb-lg-0">
        <div class="block text-center text-lg-start pe-0 pe-xl-5">
          ...
          <h1 class="text-capitalize mb-4"><?=$posts['title']?></h1>
          <p class="mb-4"><?=$posts['description']?></p> <a type="button"
            class="btn btn-primary" href="#" data-bs-toggle="modal" data-bs-target="#applyLoan"><?=$posts['btn_text']?> <span style="font-size: 14px;" class="ms-2 fas fa-arrow-right"></span></a>
        </div>
      </div>
      <div class="col-lg-6">
        <div class="ps-lg-5 text-center">
          <img loading="lazy" decoding="async"
            src="<?="./layouts/backend/assets/images/".$posts['heroImg']?>"
            alt="banner image" class="img-fluid rounded-circle border border-3 border-success" style="object-fit:cover;">
        </div>
      </div>
    </div>
  </div>
          ...
          <div class="accordion-item p-1 mb-2">
            ...
            <div id="collapse-ebd23e34fd2ed58299b32c03c521feb0b02f19d9"
              ...
              <div class="accordion-body py-0 content">The difference between and premium product consist number of
                components, assets/plugins, page in each. The Free versions contain only a few elements and pages that.</div>
            </div>
          </div>
          <div class="accordion-item p-1 mb-2">
